...
```{r prep}
library(ggplot2); library(xtable); library(slidify)
library(knitr)
opts_chunk$set(tidy = TRUE, results = 'asis', message = FALSE, 
               prompt = TRUE, dev.args=list(bg="transparent"), dpi=150,
               fig.align='center')
options(xtable.comment = FALSE, xtable.size = 10,
        xtable.type = "html") # xtable generates ugly comments by default
```
...
```{r getLetters, results='asis', warning=FALSE}
con = file("index.Rmd", open="r")
letters = toupper(unlist(strsplit(readLines(con), ""))); close(con)
letters = as.data.frame(table(gsub("[[:punct:], [:blank:], [:digit:]]", 
...
```{r plotLetters, fig.height=2, fig.width=6}
letters = letters[order(letters$Freq),]
letters$Var1 = factor(letters$Var1, as.character(letters$Var1))
ggplot(letters, aes(Var1, Freq)) + geom_point(shape = 21, size = 3) +  
